...
class TopicConsumerAll(Consumer):

    def callback(self, channel, method, properties, body) -> None:
        print("ALL: [x] Received %r" % body)
...
if __name__ == "__main__":
    ...
    with ConnectionManager() as connection_manager:
        ...
        queue = Queue(name="topic.all")

        channel.bind(queue=queue, exchange=exchange, routing_key="log.*")
        channel.basic_consume(queue=queue, consumer=TopicConsumerAll())
