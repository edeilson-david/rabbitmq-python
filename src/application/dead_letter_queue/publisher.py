...
class DirectPublisher(Publisher):
    """
    Publishes notifications to a Direct Exchange.
    """
    ...
    def publish(self, exchange: Exchange, channel: Channel) -> None:
        ...
        for notification in self.notifications:
            channel.basic_publish(
                exchange=exchange,
                routing_key=f"{notification['type']}",
...
if __name__ == "__main__":
    ...
    with ConnectionManager() as connection_manager:
        ...
        publisher.publish(exchange=DirectExchange(name="direct.notify_dl"), channel=Channel(connection_manager))
